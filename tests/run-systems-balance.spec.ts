...
test('run-level player xp timers and station values are named balance data', () => {
  ...
  expect(runBalance.xp.startingNext).toBe(42)
  expect(runBalance.xp.growthMultiplier).toBeGreaterThan(1.2)
...
test('space enemy behavior tuning is centralized outside the main game loop', () => {
  ...
  expect(spaceEnemyBehavior.rewards.xpCount.advanced).toBe(1)
