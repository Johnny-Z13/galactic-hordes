...
test('planet popups center their copy and action buttons', () => {
  ...
  expect(planet).toContain("import type { GeneratedPlanet } from '../planet-generation'")
  expect(planet).toContain('type Planet = GeneratedPlanet')
  expect(planet).toContain('interface PlanetScreenView')
  expect(planet).toContain('export function renderPlanet(self: PlanetScreenView, p: Planet)')
  expect(planet).not.toContain("from '../main'")
  expect(planet).toContain("land.textContent = p.visited ? 'Dock' : 'Land and Salvage'")
  expect(planet).toContain("land.addEventListener('click', () => runtime.confirmLanding())")
  expect(planet).toContain("runtime.showOnly('planet')")
