...
test('opening xp assist helps early rewards without buffing repairs forever', () => {
  const openingXp = pickupMagnetRange('xp', { magnetLevel: 0, limitMagnet: 0, hasHungryCompass: false, elapsed: 20 })
  const normalXp = pickupMagnetRange('xp', { magnetLevel: 0, limitMagnet: 0, hasHungryCompass: false, elapsed: 70 })
  const openingRepair = pickupMagnetRange('repair', { magnetLevel: 0, limitMagnet: 0, hasHungryCompass: false, elapsed: 20 })

  expect(openingXp).toBeGreaterThanOrEqual(420)
  expect(openingXp).toBeGreaterThan(normalXp)
  expect(openingRepair).toBe(powerupBalance.pickupMagnet.baseRange)
  expect(pickupMagnetStrength('xp', { magnetLevel: 0, limitMagnet: 0, hasHungryCompass: false, elapsed: 20 })).toBeGreaterThan(pickupMagnetStrength('xp', { magnetLevel: 0, limitMagnet: 0, hasHungryCompass: false, elapsed: 70 }))
})
