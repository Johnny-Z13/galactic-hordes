...
test('any pickup collection drives a short cyan ship absorb pulse', () => {
  const main = source()
  const pickups = pickupSource()
  const render = playerRender()

  expect(main).toContain('pickupAbsorbPulse: 0')
  expect(pickups).toContain('pickupAbsorbPulse: Math.max(input.player.pickupAbsorbPulse, 0.34)')
  expect(main).toContain('this.player.pickupAbsorbPulse = collection.player.pickupAbsorbPulse')
