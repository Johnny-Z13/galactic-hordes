...
test('hud wires route objective readout through a dedicated chip', () => {
  ...
  expect(hud).toContain("const objective = chip('ROUTE', runtime.ui.objective, 'objective wide')")
  expect(hud).toContain('const objectiveReadout = currentHudObjectiveReadout(runtime)')
  ...
  expect(hud).toContain("runtime.ui.objective.parentElement?.querySelector('.hud-label')")
