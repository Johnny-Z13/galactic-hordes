...
test('shipboard workbench installs directly from bay detail', () => {
  ...
  expect(workbench).toContain("workbenchUpgradeRows(upgrades, runtime['build']")
...
test('shipboard workbench preserves scroll position across installs', () => {
  ...
  expect(workbench).toContain("const levelup = runtime['ui'].levelup as HTMLElement")
  expect(workbench).toContain("levelup.querySelector<HTMLElement>('.workbench-panel')")
  expect(workbench).toContain("levelup.querySelector<HTMLElement>('.workbench-view')")
...
test('shipboard workbench can be skipped without spending every signal', () => {
  ...
  expect(workbench).toContain('interface WorkbenchRuntime')
  expect(workbench).toContain('interface WorkbenchHost extends Object {}')
  expect(workbench).toContain('function workbenchRuntime(self: WorkbenchHost)')
  expect(workbench).not.toContain('[key: string]: any')
  expect(workbench).not.toContain("from '../main'")
  expect(workbench).toContain('export function workbenchContinueLabel(self: WorkbenchHost)')
  expect(workbench).toContain("if (runtime['takeoffAfterWorkbench']) return 'Launch Now'")
  expect(workbench).toContain("if (runtime['returnToSectorMapAfterWorkbench']) return 'Route Map'")
  expect(workbench).toContain("return 'Resume Flight'")
  expect(workbench).toContain('export function continueFromWorkbench(self: WorkbenchHost)')
  expect(workbench).toContain("runtime['startTakeoff']({ skipWorkbench: true })")
  expect(workbench).toContain("runtime['leaveStationForSectorMap']()")
  expect(workbench).toContain('export function backFromWorkbench(self: WorkbenchHost)')
  expect(workbench).toContain("if (runtime['returnToSectorMapAfterWorkbench'] && runtime['stationDockReport'])")
  expect(workbench).toContain("runtime['state'] = 'surface'")
  expect(workbench).toContain("runtime['pendingUpgrades'] = Math.max(0, runtime['pendingUpgrades'] - 1)")
...
test('desktop mothership shows route map alongside launch controls', () => {
  ...
  expect(mothership).toContain('interface MothershipConsoleHost extends Object {}')
  expect(mothership).toContain('function mothershipRuntime(self: MothershipConsoleHost)')
  expect(mothership).not.toContain('[key: string]: any')
  expect(mothership).not.toContain("from '../main'")
  expect(mothership).toContain('export function renderMothershipRoutePreview(self: MothershipConsoleHost)')
...
test('mothership route preview nodes launch armed routes directly', () => {
  ...
  expect(mothership).toContain("runtime['launchSectorNode'](node.id)")
  expect(mothership).not.toContain("runtime['toast'](`${node.label}: ${node.description}`)")
})
...
test('mothership last report preserves the expedition story after debrief', () => {
  ...
  expect(mothership).toContain("title.textContent = runtime['debrief'].journeyTitle")
  expect(mothership).toContain("for (const highlight of runtime['debrief'].highlights.slice(0, 2))")
  expect(mothership).toContain("cargo.textContent = `Scrap ${runtime['debrief'].resources.recovered.scrap}")
...
test('mothership permanent upgrades render as selected command departments', () => {
  ...
  expect(mothership).toContain('export function renderMothershipMetaSystems(self: MothershipConsoleHost)')
...
test('mothership department upgrades preserve command scroll position', () => {
  ...
  expect(mothership).toContain("(runtime['ui'].title as HTMLElement).querySelector<HTMLElement>('.mothership-command')?.scrollTop")
