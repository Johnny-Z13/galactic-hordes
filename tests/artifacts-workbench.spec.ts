...
test('shipboard workbench preserves scroll position across installs', () => {
  ...
  expect(workbench).toContain("const levelup = self['ui'].levelup as HTMLElement")
...
test('shipboard workbench can be skipped without spending every signal', () => {
  ...
  expect(workbench).toContain('export function workbenchContinueLabel(self: WorkbenchRuntime)')
  ...
  expect(workbench).toContain('export function continueFromWorkbench(self: WorkbenchRuntime)')
  expect(workbench).toContain("self['startTakeoff']({ skipWorkbench: true })")
  expect(workbench).toContain("self['leaveStationForSectorMap']()")
  expect(workbench).toContain('export function backFromWorkbench(self: WorkbenchRuntime)')
